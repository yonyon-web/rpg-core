...
戦闘や成長のシミュレーション機能に関する設計。Core Engine、Serviceの2層で構成。
...
シミュレーションシステムは以下の機能を提供します：

- **戦闘シミュレーション**: 自動戦闘の実行と結果予測
- **確率計算**: 各種判定の確率と期待値の算出
- **ダメージ期待値**: 平均ダメージとクリティカル込みの期待値計算
...
この設計により、GEasy-Kitライブラリの15のServiceを一貫性を持って実装できます。
