...
// 後方互換: ExtensibleConfigから装備スロット構成を取得する例
...
**装備スロット構成について**:
- **推奨**: Core Engineの `getEquipmentSlots()` メソッドを使用してスロット構成を取得
- デフォルト構成は Core Engine 側で定義（`DEFAULT_EQUIPMENT_SLOTS`）
