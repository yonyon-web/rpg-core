...
戦闘に関する包括的な設計。Core Engine、Service、Headless UIの3層で構成。
...
- **ダメージ計算**: 物理攻撃、魔法攻撃、回復などの計算
- **行動順管理**: 素早さに基づく行動順の決定
- **コマンド選択**: プレイヤーの戦闘コマンド選択フロー
- **勝敗判定**: 戦闘の勝利・敗北・逃走の判定
...
┌─────────────────────────────────────┐
│  Headless UI Layer                  │
│  BattleController                   │  ← UI状態管理、ユーザー操作
│  CommandController                  │
│  - 表示状態の管理                    │
│  - コマンド選択の管理                │
└─────────────────────────────────────┘
